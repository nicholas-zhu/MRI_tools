...
netG = define_G(opt.input_cn, opt.output_cn, 64, 'ResUNetModule',resnet_output_idx = opt.input_cn//2)
...
criterion_sim7 = SSIM_loss1(window_size = 5,non_neg_flag = False)
...
for epoch in range(opt.epoch, opt.n_epochs):
    for dataname_t in datalist_train:
        ...
        for i, batch in enumerate(dataloader):
            ...
            fake_B = netG(real_A)
            ref_B1 = netG(ref_B)
            loss_sim = criterion_sim7(real_B, fake_B, mask)*5.0
            loss_id = criterion_l1((fake_B+1e-6),(real_B+1e-6))
            loss_id2 = criterion_l1((ref_B1+1e-6),(real_B+1e-6))
